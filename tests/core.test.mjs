...
run("state=fresh();state.step=4;action('sample');state.step=5;action('sample');sim.form={}");const hidden=run('preview(section())');assert.ok(!hidden.includes('data-form-key="order_number"'));run("sim.form={reason:'Livraison::Retard'}");assert.ok(run('preview(section())').includes('data-form-key="order_number"'));
run("state.step=6;action('sample')");assert.equal(run('issues(section()).length'),0);run("d('views').rows[0].title='a::b::c::d'");assert.ok(run('issues(section()).length')>0);
assert.equal(run("esc('<script>')"),'&lt;script&gt;');assert.ok(run("tree(['__proto__::x','_leaf::y'])").includes('_leaf'));assert.ok(run("safeCSV('=CMD()')").startsWith('"\''));ctx.pasted='a\t"b\nc"\r\nd\te';assert.equal(run('parseDelimited(pasted)[0][1]'),'b\nc');assert.throws(()=>run('validate({schemaVersion:9})'));
run('persist();state=validate(JSON.parse(localStorage.getItem(KEY)))');assert.equal(run('state.schemaVersion'),1);
run("state=fresh();state.step=4;action('sample');download=(name,data)=>{globalThis.exported=JSON.parse(data)};exportAPI()");assert.equal(run('exported.requests[0].body.ticket_field.type'),'tagger');assert.equal(run('exported.requests[0].body.ticket_field.required_in_portal'),true);assert.equal(run('exported.notExecuted'),true);
